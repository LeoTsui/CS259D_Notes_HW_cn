...
* $$s$$，按照用户 ID 和时间戳排序的序列
    * $$s = \{s_1, s_2, ..., s_k\}$$，其中每个 $$s_j$$
        * 一个 $$\text{operation-set}$$ （$$I$$ 的子集）， $$s_j \subset I$$，其中 $$1 \leq j \leq l$$
        * 被称为序列 $$s$$ 的一个元素
        * $$s_j = \langle x_1, x_2, ..., x_m \rangle$$，其中每个 $$x_k$$ 是一个鼠标操作
    * 例如：
        * $$s = \{\langle(1,3,4,0)\rangle, \langle(1,3,4,0), (2,1,4,0), (3,1,4,0)\rangle, \langle(2,1,2,1)\rangle\}$$
        * $$\mathrm{length}(s) = 5$$
* $$\alpha$$，$$s$$ 的一个子序列
    * 例如：
        * $$\{\langle(1,3,4,0), (2,1,4,0), (3,1,4,0)\rangle\}$$ 的一个子序列是 $$\{\langle(1,3,4,0)\rangle\}$$
* $$L$$，序列长度：鼠标操作实例的个数
